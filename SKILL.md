...
- **Hold / insufficient evidence** — use this in either of two cases: (a) the idea fails 1–2 questions and **no credible fix is identifiable yet**; or (b) too much of the table is unresolved to make an evidentiary verdict at all (for example, 3+ load-bearing questions or 5+ total questions remain [Assumption]/low-confidence). Don't convert a mostly unresearched candidate into artificial passes or fails just to satisfy a threshold. Name the specific unresolved question(s) and route to the highest-information next test; if the gap is broad, prescribe a short evidence-gathering sequence rather than pretending one test will resolve everything. Re-run Stage 3 after that sequence, rather than proceeding on a guess.
...
**Stage 4 is not the default next step for every non-clean verdict.** Enter it only when a killed, held, or advance-with-a-condition verdict rests on a tar-pit or other structural mechanism. An evidence-gap hold goes directly to its named evidence-gathering sequence and then back to Stage 3; do not reframe an idea merely because it is unresearched. For structural cases, ask whether a reframe genuinely dissolves the problem before discarding the idea. Apply this rule strictly:
